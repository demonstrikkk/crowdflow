"""Deterministic GLB (binary glTF 2.0) exporter for the generated venue.

The 3D model is produced from the semantic/spatial venue model — never from
free-form AI geometry. It uses the same world transform as the frontend twin
renderer (``twinGeometry.makeFrame``) so that simulation overlays, gate
markers and the GLB share one coordinate frame:
...
bind GLB nodes to simulation elements. The writer is dependency-free (pure
``struct`` + ``json``) so it also runs inside the Colab worker.
"""
...
def write_glb(meshes: List[GlbMesh], output_path: Optional[str] = None) -> bytes:
    """Build a minimal, valid binary glTF 2.0 file (flat-shaded, no textures).

    Returns the GLB bytes; also writes them to ``output_path`` when given.
    """
